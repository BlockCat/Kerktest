...
data class Answer(val text: String,val influence: List<Church> = emptyList(), var important: Int = 1)
...
fun getQuestions(): List<Question> =
        listOf(
            ...
            Question("Ben je momenteel lid van een kerk?", listOf(
                    listOf(Answer("Nee", listOf())),
                    Church.values().map { Answer("Ja: ${it.n}") }
            ).flatten()),
            Question("Welke stroming verwacht je dat de test je zal kwalificeren?",
                    Church.values().map { Answer(it.n) })
        )